...
void SelfRobot::publishSelfState(g2o::SparseOptimizer* graph_ptr)
{
    ...
    estimatedTargetPose.header.stamp = curTime;
    estimatedTargetPose.header.frame_id = "/world"; 
...
// 	//cout<<"latestOptimizedRobPoseVer = "<<latestOptimizedRobPoseVer<<endl;
    ...
    estimatedTargetPose.point.x =  targetsToTrack[0]->curPose[0];
    estimatedTargetPose.point.y =  targetsToTrack[0]->curPose[1];
    estimatedTargetPose.point.z =  targetsToTrack[0]->curPose[2];
...
{
  ros::init(argc, argv, "coop_loc_track");
  ros::NodeHandle nh("~");
